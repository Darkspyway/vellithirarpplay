...
const state = {
    ...
    applicantName: "",
    discordUser: null,
    answers: [] // Stores { q: string, a: string }
};
...
function checkDailyLimit() {
    ...
    const storedData = JSON.parse(localStorage.getItem('vt_attempts') || '{"date": "", "count": 0}');
    
    // If date is different, reset count
    if (storedData.date !== today) {
        return;
    }

    const attemptsLeft = 3 - storedData.count;
    if (attemptsLeft <= 0) {
        ...
        dailyLimitStatus.style.color = '#ff4757';
        dailyLimitStatus.innerHTML = '<i class="fas fa-exclamation-circle"></i> Daily limit reached (3/3 attempts). Please try again tomorrow.';
    } else {
        dailyLimitStatus.style.display = 'block';
        dailyLimitStatus.style.color = '#ffa502';
        dailyLimitStatus.innerHTML = `<i class="fas fa-clock"></i> You have <b>${attemptsLeft}</b> attempts remaining for today.`;
    }
}
...
function startExam() {
    ...
    localStorage.setItem('vt_attempts', JSON.stringify(storedData));
...
function selectOption(el, isCorrect) {
    ...
    state.lastSelectedText = el.querySelector('.option-text').innerText;
...
function handleNext() {
    ...
    if (q.type === 'mcq') {
        if (state.lastSelectedCorrect) state.score += 10;
        state.answers.push({ q: q.q, a: state.lastSelectedText || "No answer" });
    } else {
        const text = essayInput.value.trim();
        if (text.length > 50) state.score += 10; 
        state.answers.push({ q: q.q, a: text || "No answer" });
    }
...
function finishExam() {
    ...
    if (state.failed) {
        ...
        resultIconEl.innerHTML = '<i class="fas fa-clock icon-pass" style="color: var(--primary);"></i>';
        resultTitleEl.innerText = "APPLICATION SUBMITTED";
        resultMsgEl.innerText = "Your answers have been sent to our staff for manual review. You will be notified via Discord DM once a decision is made.";
        failReasonEl.innerText = "Current Status: Under Review";
    }

    // NO LIMIT FOR INTERVIEW: All completed exams go to staff for manual review
    if (!state.failed) {
        assignRole(finalScore, state.answers);
    } else {
        logFailure(finalScore, state.answers);
    }
}

async function logFailure(score, answers) {
    try {
...
async function assignRole(score, answers) {
    console.log('📤 Submitting Application:', { score, answersCount: answers?.length });
    
    const resultMsg = document.getElementById('result-msg');
    const originalText = resultMsg.innerText;
    resultMsg.innerHTML = `<i class="fas fa-spinner fa-spin"></i> Submitting for review...`;
