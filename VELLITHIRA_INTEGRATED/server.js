...
const STAFF_ROLE_IDS = (process.env.DISCORD_STAFF_ROLE_IDS || "").split(',').map(id => id.trim()).filter(id => id);
...
const PASS_LOG_CHANNEL = process.env.DISCORD_PASS_LOG_CHANNEL || '📑-manual-review';
const FAIL_LOG_CHANNEL = process.env.DISCORD_FAIL_LOG_CHANNEL || '❌-fail-logs';
const ACCEPTED_LOG_CHANNEL = process.env.DISCORD_ACCEPTED_LOG_CHANNEL || '✅-accepted-citizens';
const MOD_LOG_CHANNEL = process.env.DISCORD_MOD_LOG_CHANNEL || '🛡️-mod-logs';
...
async function sendLog(channelIdOrName, embedData, content = '') {
    try {
...
// Helper function to check if a member has staff permissions
function checkStaffPermission(member) {
    if (!member) return false;
    // If no staff roles are configured, allow all (to avoid locking out)
    if (STAFF_ROLE_IDS.length === 0) return true;
    return member.roles.cache.some(role => STAFF_ROLE_IDS.includes(role.id));
}

// Match Button Interactions (Accept/Reject)
client.on('interactionCreate', async (interaction) => {
    // Check permissions for staff-only actions
    const isStaffAction = interaction.isButton() || interaction.isModalSubmit();
    if (isStaffAction && interaction.customId.includes('application')) {
        const guild = await interaction.guild.fetch();
        const member = await guild.members.fetch(interaction.user.id);
        
        if (!checkStaffPermission(member)) {
            return interaction.reply({ 
                content: '🛡️ **UNAUTHORIZED:** You do not have the required staff roles to process applications.', 
                ephemeral: true 
            });
        }
    }
    ...
    if (interaction.isModalSubmit()) {
        ...
        if (action === 'reject' && type === 'application') {
            ...
            try {
                ...
                if (member) {
                    try {
                        const failEmbed = new EmbedBuilder()
                            ...
                            .setDescription(`Hello <@${member.id}>,\n\nWe regret to inform you that your application for **Vellithira Roleplay** has been **REJECTED** after manual review by our staff.\n\n**Reason:** ${reason}\n\nPlease review our rules and try again after your daily cooldown.`)
                            .setColor(0xff4757)
                            .setTimestamp()
                            .setFooter({ text: 'Vellithira RP • Citizenship System' });
                        await member.send({ embeds: [failEmbed] });
                    } catch (dmErr) { console.warn('Could not send DM to member:', dmErr.message); }
                }
                ...
                const embed = EmbedBuilder.from(interaction.message.embeds[0])
                    ...
                    .addFields(
                        { name: 'Processed By', value: `${staffName}`, inline: true },
                ...
                // Log to Mod Logs
                await sendLog(MOD_LOG_CHANNEL, {
                    title: '🛡️ STAFF ACTION: REJECT',
                    description: `**Staff:** ${staffName}\n**Applicant:** <@${targetId}>\n**Status:** Application Denied\n**Reason:** ${reason}`,
                    color: 0xff4757,
                    timestamp: new Date()
                });

                // Log to Rejection Logs (FAIL_LOG_CHANNEL)
                await sendLog(FAIL_LOG_CHANNEL, {
                    title: '🔻 APPLICATION REJECTED',
                    description: `An application has been manually rejected after staff review.`,
                    fields: [
                        { name: 'Applicant', value: `<@${targetId}>`, inline: true },
                        { name: 'Processed By', value: `${staffName}`, inline: true },
                        { name: 'Reason', value: reason }
                    ],
                    color: 0xff4757,
                ...
                if (!interaction.replied) await interaction.reply({ content: '❌ Error processing rejection.', ephemeral: true });
            }
        }
        return;
    }
    ...
    try {
        ...
        if (action === 'accept') {
            if (member) {
                ...
                try {
                    const passEmbed = new EmbedBuilder()
                        .setTitle('✨ YOUR VISA HAS BEEN APPROVED!')
                        .setDescription(`Congratulations <@${member.id}>! Your application for **Vellithira Roleplay** has been **ACCEPTED**.\n\nYour Visa role has been assigned. You can now join the city and start your journey!`)
                        .setColor(0x00ff99)
                        .setTimestamp()
                        .setFooter({ text: 'Vellithira RP • Citizenship System' });
                    await member.send({ embeds: [passEmbed] });
                } catch (dmErr) { console.warn('Could not send DM to member:', dmErr.message); }
            }
            ...
            const embed = EmbedBuilder.from(interaction.message.embeds[0])
                ...
                .addFields({ name: 'Processed By', value: `${staffName}`, inline: true });

            await interaction.update({ embeds: [embed], components: [] });
            
            // Log to Accepted Citizens Log
            await sendLog(ACCEPTED_LOG_CHANNEL, {
                title: '💎 NEW CITIZEN ACCEPTED',
                description: `A new citizen has been officially accepted into Vellithira!`,
                fields: [
                    { name: 'Citizen', value: `<@${targetId}>`, inline: true },
                    { name: 'Processed By', value: `${staffName}`, inline: true }
                ],
                color: 0x00ff99,
                timestamp: new Date()
            ...
            // Log to Mod Logs
            await sendLog(MOD_LOG_CHANNEL, {
                title: '🛡️ STAFF ACTION: ACCEPT',
                description: `**Staff:** ${staffName}\n**Applicant:** <@${targetId}>\n**Status:** Visa Role Assigned`,
                color: 0x00ff99,
                timestamp: new Date()
            });
            ...
            const modal = new ModalBuilder()
                .setCustomId(`reject_application_${targetId}`)
                .setTitle('Rejection Reason');

            const reasonInput = new TextInputBuilder()
                .setCustomId('reject_reason')
                .setLabel("Why are you rejecting this player?")
                .setStyle(TextInputStyle.Paragraph)
                .setPlaceholder('Enter reason (e.g., Weak essay, Poor rule knowledge)')
                .setRequired(true);
        ...
        if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({ content: '❌ Error processing decision.', ephemeral: true });
        }
    }
});

function formatAnswers(answers) {
    if (!answers) return '⚠️ No answers object received in request.';
    if (!Array.isArray(answers)) return `⚠️ Answers received but not in array format (Type: ${typeof answers}).`;
    if (answers.length === 0) return '⚠️ Answers array is empty.';
    
    return answers.map((a, i) => `**Q${i + 1}:** ${a.q}\n**A:** ${a.a}`).join('\n\n').substring(0, 3800);
}
...
client.once('ready', async (c) => {
    ...
    const commands = [
        new SlashCommandBuilder()
            .setName('update_tutorial')
            .setDescription('Update a tutorial video on the website')
            .addStringOption(option =>
                option.setName('category')
                    .setDescription('The tutorial category to update')
                    .setRequired(true)
                    .addChoices(
                        { name: 'City Basics', value: 'city_basic' },
                        { name: 'Healing & Survival', value: 'heal' },
                        { name: 'Robbery & Heists', value: 'rob' },
                        { name: 'Underground Grinding', value: 'grind' },
                        { name: 'EMS Guide', value: 'ems_guide' },
                        { name: 'PD Protocol', value: 'pd_guide' },
                        { name: 'Drug Lab', value: 'bm_drugs' },
                        { name: 'Hacking/Breach', value: 'bm_hacking' },
                        { name: 'Navigation/GPS', value: 'city_map' },
                        { name: 'Labor Market', value: 'job_center' },
                        { name: 'Money Laundry', value: 'bm_money' },
                        { name: 'Illegal Armory', value: 'bm_arms' }
                    ))
            .addStringOption(option =>
                option.setName('link')
                    .setDescription('The YouTube/Video link (e.g., https://youtu.be/...)')
                    .setRequired(true))
            .toJSON(),
        new SlashCommandBuilder()
            .setName('website')
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.commandName === 'update_tutorial') {
        ...
        const index = tutorials.findIndex(t => t.id === category);

        // Smarter URL parsing for YouTube
        let videoEmbed = link;
        ...
        if (index !== -1) {
            ...
            const embed = new EmbedBuilder()
                .setTitle('🎥 TUTORIAL UPDATED')
                .setDescription(`The tutorial for **${tutorials[index].title}** has been successfully updated on the website.`)
                .addFields(
                    { name: 'Category', value: tutorials[index].category, inline: true },
            ...
            // Log to Mod Logs
            await sendLog(MOD_LOG_CHANNEL, {
                title: '🛡️ STAFF ACTION: TUTORIAL UPDATE',
                description: `**Staff:** ${interaction.user.tag}\n**Tutorial:** ${tutorials[index].title}\n**Link:** ${link}`,
                color: 0x3498db,
                timestamp: new Date()
            });
        } else {
            await interaction.reply({ content: `❌ Tutorial category **${category}** not found in tutorials.json.`, ephemeral: true });
        }
    }
...
app.post('/api/pass', async (req, res) => {
    const { discordUsername, score, answers } = req.body;
    console.log(`  📩 New Submission: ${discordUsername} | Score: ${score}% | Answers: ${answers ? answers.length : 'MISSING'}`);
    ...
    try {
        ...
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`accept_application_${member.id}`).setLabel('Accept').setStyle(ButtonStyle.Success).setEmoji('✅'),
            new ButtonBuilder().setCustomId(`reject_application_${member.id}`).setLabel('Reject').setStyle(ButtonStyle.Danger).setEmoji('❌')
        );

        const embed = new EmbedBuilder()
            .setTitle('📑 NEW APPLICATION FOR REVIEW')
            .setDescription(`**Applicant:** <@${member.id}>\n**Score:** ${score}%\n\n**-- Answers --**\n${answerText}`)
            .setColor(0x3498db)
            .setTimestamp();

        const channel = await resolveChannel(guild, PASS_LOG_CHANNEL);
        if (channel) await channel.send({ content: '🔔 New application received!', embeds: [embed], components: [row] });

        return res.json({ success: true, message: `Submitted! Staff will review soon.` });
    } catch (error) {
        console.error('API Pass error:', error.message);
...
app.post('/api/fail', async (req, res) => {
    const { discordUsername, score, reason, answers } = req.body;
    if (!discordUsername) return res.status(400).json({ success: false, message: 'Username required.' });
    ...
    try {
        ...
        const answerText = formatAnswers(answers);
        const applicantLabel = member ? `<@${member.id}>` : discordUsername;

        await sendLog(FAIL_LOG_CHANNEL, {
            title: '🔻 AUTO-REJECTION (Failed Exam)',
            description: `**Applicant:** ${applicantLabel}\n**Score:** ${score}%\n**Reason:** ${reason}\n\n**-- Answers --**\n${answerText}`,
            color: 0xff4757,
            timestamp: new Date()
        }, member ? `<@${member.id}>` : '');
        ...
        console.error('API Fail error:', error.message);
        return res.status(500).json({ success: false });
    }
});
